...
const updateReview = async (req,res)=>{
    try{
        ...
        if(req.body.rating){
        if(typeof req.body.rating !== "number" ||  (req.body.rating <= 1) || (req.body.rating > 5) ) return res.status(400).send({ status: false, message: "you can rate only 1 to 5 " });
        }
        if(req.body.rating === 0 ) return res.status(400).send({ status: false, message:"you can rate only 1 to 5 " }); 
